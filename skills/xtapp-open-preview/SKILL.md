...
Use `get_xtapp_preview_status` to refresh the widget and confirm the page. Use `watch_xtapp_preview` only to stop or restart the watcher. Use `sync_xtapp_preview_source` for a one-off source push, `send_xtapp_preview_input` for keys, and `send_xtapp_preview_touch` as the default click. Use `tap_xtapp_preview_target` only when `/preview/targets` already lists a rectangle; if it is empty, capture a PNG and click coordinates. Do not add a Lua testing slot just to tap. Nested `domain/` Lua is synced. `capture_xtapp_preview` takes a PNG, `restart_xtapp_preview` is a manual retry, and `stop_xtapp_preview` stops. Diagnose with `inspect_xtapp_preview_context` plus a contract search. Do not start a second preview server.
